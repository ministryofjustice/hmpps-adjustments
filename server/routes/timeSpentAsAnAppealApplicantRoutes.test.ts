...
describe('Time spent as an appeal applicant not to count routes', () => {
  ...
  `(
    ...
    async ({ addOrEdit }) => {
      ...
      return request(app)
        ...
        .expect(302)
        .expect('Location', `/${NOMS_ID}/appeal-applicant/offences/${addOrEdit}/${SESSION_ID}`)
    },
  )
  ...
  test.each`
    ...
    ${'add'}
    ${'edit'}
  `('GET /{nomsId}/appeal-applicant/offences/add displays the correct information', async ({ addOrEdit }) => {
    adjustmentsStoreService.getById.mockReturnValue(timeSpentAsAnAppealApplicantAdjustment)
    prisonerService.getSentencesAndOffencesFilteredForRemand.mockResolvedValue(stubbedSentencesAndOffences)
    return request(app)
      .get(`/${NOMS_ID}/appeal-applicant/offences/${addOrEdit}/${SESSION_ID}`)
      .expect(200)
      .expect('Content-Type', /html/)
      .expect(res => {
  ...
  test.each`
    ...
    ${'edit'}
  `(
    'POST /{nomsId}/appeal-applicant/offences/:addOrEdit no offences are selected results in an error',
    async ({ addOrEdit }) => {
      prisonerService.getSentencesAndOffencesFilteredForRemand.mockResolvedValue(stubbedSentencesAndOffences)
      adjustmentsStoreService.getById.mockReturnValue(timeSpentAsAnAppealApplicantAdjustment)
      return request(app)
        .post(`/${NOMS_ID}/appeal-applicant/offences/${addOrEdit}/${SESSION_ID}`)
        .send()
        .expect('Content-Type', /html/)
        .expect(res => {
          expect(res.text).toContain(
            'You must select the offence(s) which relate to the time spent as an appeal applicant not to count adjustment.',
          )
        })
    },
  )
  ...
  test.each`
    ...
    ${'edit'}
  `(
    'POST /{nomsId}/appeal-applicant/offences/:addOrEdit selected offences redirects to review',
    async ({ addOrEdit }) => {
      prisonerService.getSentencesAndOffencesFilteredForRemand.mockResolvedValue(stubbedSentencesAndOffences)
      adjustmentsStoreService.getById.mockReturnValue(timeSpentAsAnAppealApplicantAdjustment)
      return request(app)
  ...
  test.each`
    ...
    ${'offences'}  | ${'add'}  | ${`/${NOMS_ID}/appeal-applicant/reference/add/${SESSION_ID}" class="govuk-back-link"`}
    ${'offences'}  | ${'edit'} | ${`/${NOMS_ID}/appeal-applicant/reference/edit/${SESSION_ID}" class="govuk-back-link"`}
    ${'review'}    | ${'add'}  | ${`/${NOMS_ID}/appeal-applicant/offences/add/${SESSION_ID}" class="govuk-back-link"`}
    ${'review'}    | ${'edit'} | ${`/${NOMS_ID}/appeal-applicant/offences/edit/${SESSION_ID}" class="govuk-back-link"`}
