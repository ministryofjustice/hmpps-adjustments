...
`('GET /{nomsId}/special-remission/type/:addOrEdit has the correct text on it', async ({ addOrEdit }) => {
  ...
  return request(app)
    ...
    .expect(res => {
      ...
      expect(res.text).toContain(
        'The balance of the sentence to be served to the correct release date has been cancelled out. The person will be released on an earlier date. Release in error',
      )
      ...
      expect(res.text).toContain('You can read the rules for special remission')
      expect(res.text).toContain(
...
test.each`
  ...
  ${'add'}
  ${'edit'}
`('POST /{nomsId}/special-remission/type/:addOrEdit valid input directs to the review page', async ({ addOrEdit }) => {
  adjustmentsStoreService.getById.mockReturnValue(blankAdjustment)
  return request(app)
    .post(`/${NOMS_ID}/special-remission/type/${addOrEdit}/${SESSION_ID}`)
